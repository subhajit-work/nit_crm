...
@Component({
  ...
  templateUrl: './payment-add.page.html',
  styleUrls: ['./payment-add.page.scss'],
})
export class PaymentAddPage implements OnInit,OnDestroy {
  ...
  studentList: any;
  paymentStatus: any;
  paymentStatusName='Due';
  paymentStatusNamedis=false;
  constructor(
...
//--------------  getlist data fetch start -------------
  ...
  paymentmonthList=[
    ...
    commonFunction(){
      ...
      this.getlistStudent('get-student');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_show_api = 'get-payment-id?id='+this.parms_action_id;
        this.form_api = 'edit-payment?id='+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'add-payment'
      }
      ...
      this.editDataSubscribe = this.http.get(this.form_show_api).subscribe(
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              student_name : this.from_data_show.student_id,
              course_name : this.from_data_show.course_id,
              payment_month : this.from_data_show.payment_month,
              payment_type : this.from_data_show.payment_type,
              payment_id : this.from_data_show.payment_id,
              payment_date : this.from_data_show.payment_date,
              payment_remark : this.from_data_show.payment_remark
            }; 
            ...
            this.onChange(this.model.student_name);
            // edit data
            this.model.due_amount=this.from_data_show.amount;
  ...
  // ---------- student data fetch end ----------
  getlistStudent(_getlistUrl){
    this.plt.ready().then(() => {
      this.selectLoading = true;
      this.getListSubscribe = this.commonUtils.getlistCommon(_getlistUrl).subscribe(
        resData => {
...
// getlist data fetch end
  ...
  onPaymentId(_type)
  {
    ...
    if(_type=='Cash')
    {
      this.model.payment_id=Math.random()*10000000009;
      this.model.payment_id=parseInt(this.model.payment_id);
  ...
  onChange(_item){
    ...
    this.studentList.forEach(element => {
     if(element.id==_item)
     {
       this.model.course_name=element.course_id;
       dis=element.discount_amount;
    ...
    this.courseList.forEach(element => {
      if(element.id==this.model.course_name)
      {
        // this.model.total_amount=((element.total_amount-dis)/element.total_amount)  * 100;
        this.model.total_amount=(element.total_amount-(element.total_amount* dis)/100);
        this.model.total_amount=parseInt(this.model.total_amount);
        if(due==0)
        {
          this.model.due_amount=this.model.total_amount;
        }
        else if(due==this.model.total_amount)
        {
          this.model.due_amount=this.model.total_amount;
          this.paymentStatusName='Paid';
          this.paymentStatusNamedis=true;
        }
        else
        {
          this.model.due_amount=this.model.total_amount-due;
        }
