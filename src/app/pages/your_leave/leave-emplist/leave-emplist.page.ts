...
@Component({
  selector: 'app-leave-emplist',
  templateUrl: './leave-emplist.page.html',
  styleUrls: ['./leave-emplist.page.scss'],
})
export class LeaveEmplistPage implements OnInit, OnDestroy {
  ...
  private leaveDataSubscribe: Subscription;
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        console.log('common');
        this.getLeave();        // get active url name
        ...
        this.headerUrlapi = 'leave-header';
    
        // table list data url name
        this.listing_url = 'leave-employee-list';

        // delete url
        this.delete_url = 'leave-delete';
    ...
    leaveData;
    getLeave(){
      this.leave_Url = 'leave-data';
      this.leaveDataSubscribe = this.http.get(this.leave_Url).subscribe(
        (res:any) => {
          if(res.return_status > 0){
            this.leaveData = res.return_data;
            console.log("Leave return data..",this.leaveData);
          }
        },
        errRes => {
    ...
    // --------------- enable disable call start ---------------------
      ...
      enabledDisabled( _identifire, _item, _items ){
        ...
        this.itemsSubscribe = this.http.get('course-status?status='+_item+'&id='+_items).subscribe(
          (res:any) => {
            if(res.return_status > 0)
            {
    ...
    // ----------- destroy subscription start ---------
      ngOnDestroy() {
        ...
        if(this.leaveDataSubscribe !== undefined){
          this.leaveDataSubscribe.unsubscribe();
        }
      }
