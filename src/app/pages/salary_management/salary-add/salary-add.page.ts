...
@Component({
  ...
  templateUrl: './salary-add.page.html',
  styleUrls: ['./salary-add.page.scss'],
})
export class SalaryAddPage implements OnInit, OnDestroy {
  ...
  emp_id: any;
    ...
    commonFunction(){
      ...
      if( this.parms_action_name == 'view'){
        ...
        this.form_show_api = 'salary-view/'+this.parms_action_id;
      }else if(this.parms_action_name == 'edit'){
        this.form_show_api = 'salary-view/'+this.parms_action_id;
        this.form_api = 'salary-edit/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'salary-add';
      }
    ...
    if( this.parms_action_name == 'view'){
      ...
      this.editDataSubscribe = this.http.get(this.form_show_api).subscribe(
        (res:any) => {
          ...
          this.fname = res.return_data.employee.first_name;
          this.lname = res.return_data.employee.last_name;
          if(res.return_status > 0){
            this.model = {
              employee: res.return_data.employee.id,
              roll: res.return_data.roll.id,
              designation: res.return_data.designation.id,
              basic: res.return_data.basic,
              house_rent: res.return_data.house_rent,
              conveyance: res.return_data.conveyance,
              medical: res.return_data.medical,
              special: res.return_data.special,
              income_tax: res.return_data.income_tax,
              incentive: res.return_data.incentive,
            }; 
