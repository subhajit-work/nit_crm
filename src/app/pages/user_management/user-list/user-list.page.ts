...
import { PopoverController, Platform, ModalController, MenuController, AlertController, NavController } from '@ionic/angular';
...
export class UserListPage implements OnInit, OnDestroy {
  ...
  private cardSubscribe: Subscription;
  ...
  constructor(
    ...
    public orderChartColor = [
      "#009846","#00a0e3","#ef7f1a","#b0cb1f"
    ];
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'profile-header';
    
        // table list data url name
        this.listing_url = 'employee-list';

        // delete url
        this.delete_url = 'employee-delete';
    ...
    // ------ Get Cards Data ------
    getDashboardcard(){
      this.card_Url = 'employee-dashboard';
      this.cardSubscribe = this.http.get(this.card_Url).subscribe(
        (res:any) => {
          if(res.return_status > 0){
            console.log("Cards_data>>>>>",res);
            this.cardsData = res.return_data;
    ...
    onListData(_list_url, _displayRecord, _page, _apiParms, _search, _cherecterSearch, _orderBy, _order, _advSearchParms, _identifire) {
      this.plt.ready().then(() => {
        ...
        this.itemsSubscribe = this.commonUtils.fetchList(_list_url, _displayRecord, _page, _apiParms,  _search, _cherecterSearch, _orderBy, _order, _advSearchParms, _identifire).subscribe(
          resData => {
            console.log("List Data Not Call..............");
    ...
    // -------- pagination -------------
      ...
      setPage(page: number) {
        ...
        this.pager = this.pagerService.getPager(this.listAlldata, page, this.displayRecord);
    ...
    // ---------------- Click Delete Item start ---------------------
      ...
      async onClickDeleteItem(_identifire, _item, _id){
          ...
          buttons: [
            {
              ...
              handler: () => {
                ...
                if(_identifire == 'single'){
                  ...
                  this.deleteListSubscribe = this.http.post(this.delete_url+'/'+_id, '').subscribe(
                  (res:any) => {
   ...
   // enable disable call end
    ...
    // ----------- destroy subscription start ---------
      ngOnDestroy() {
        ...
        if(this.cardSubscribe !== undefined){
          this.cardSubscribe.unsubscribe();
        }
      }
