  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      if( this.parms_action_name == 'view'){
        ...
        this.form_show_api = 'task-view/'+this.parms_action_id;
      }else if(this.parms_action_name == 'edit'){
        this.form_show_api = 'task-view/'+this.parms_action_id;
        this.form_api = 'task-edit/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'task-add'
      }
    ...
    if( this.parms_action_name == 'view'){
      ...
      this.editDataSubscribe = this.http.get(this.form_show_api).subscribe(
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              project_id : res.return_data.project_id.id,
              employee_id : res.return_data.employee_id.id,
              start_time : res.return_data.start_time,
              end_time : res.return_data.end_time,
      ...
      this.editDataSubscribe = this.http.get(this.form_show_api).subscribe(
        (res:any) => {
          ...
          console.log("Edit data  res >", res.return_data);
          this.from_data_show = res.return_data;
          this.fname = res.return_data.employee_id.first_name;
          this.lname = res.return_data.employee_id.last_name;
          if(res.return_status > 0){
            this.model = {
              project_id : res.return_data.project_id.id,
              employee_id : res.return_data.employee_id.id,
    ...
    (response:any) => {
      ...
      if(response.return_status > 0){
        ...
        if(this.parms_action_name == 'add' || this.parms_action_name == 'edit'){
          this.router.navigateByUrl(`task-list/`+this.model.project_id);
        }
