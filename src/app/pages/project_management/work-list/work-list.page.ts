...
export class WorkListPage implements OnInit, OnDestroy {
  ...
  private taskStatusSubscribe: Subscription;
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'task-employee-header';
        // table list data url name
        this.listing_url = 'get-task';
        // delete url
    ...
    ionViewWillEnter() {
      ...
      taskData;
      getTaskReview(){
        this.task_Url = 'task-all-status';
        this.taskStatusSubscribe = this.http.get(this.task_Url).subscribe(
          (res:any) => {
            if(res.return_status > 0){
              this.taskData = res;
              console.log("Task Status return data..",this.taskData);
            }
          },
          errRes => {
    ...
    statusChangeLoading;
    statusChange( _id, _item ){
      console.log("task status id....",_id);
      
      this.statusChangeLoading = true;
      this.statuschange = 'task-status-update';
      let fd = new FormData();
      fd.append('project_status', _item);
      fd.append('task_id', _id);
      
      this.itemsSubscribe = this.http.post(this.statuschange, fd).subscribe(
        (res:any) => {
          if(res.return_status > 0)
          {
  ...
  // enable disable call end
    ...
    // ----------- destroy subscription start ---------
      ngOnDestroy() {
        ...
        if(this.taskStatusSubscribe !== undefined){
          this.taskStatusSubscribe.unsubscribe();
        }
      }
