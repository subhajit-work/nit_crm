...
export class LeaveRequestListPage implements OnInit {
  ...
  commonFunction() {
    ...
    this.listing_url = 'get-leave-request';
  ...
  statusChangeLoading;
  async enabledDisabled(_identifire, _item, _items) {
    // console.log(_items);
    // console.log(_item);
      ...
      buttons: [
        ...
        },
        {
          ...
          handler: (alertData) => {
            ...
            this.itemsSubscribe = this.http.get('leave-status?status=' + _item + '&id=' + _items + '&message=' + alertData.reason).subscribe(
              (res: any) => {
                if (res.return_status > 0) {
  ...
  //popup alert or description section start by apurba
