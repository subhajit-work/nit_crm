...
export class AuthService {
      ...
      return from(this.storage.get('setStroageGlobalParamsData')).pipe(
        map(storData => {
          ...
          const storeauth = {
            ...
            'login_data': storData.login_data,
            'time_msg': storData.time_msg
          }
          return storeauth;
        }),
  ...
  // ================= login service call start ==================
    login(_url, _data, _siteInfo) {
      return this.http.post(`${_url}?master=`+_siteInfo.id, _data).pipe(
      ...
      this._globalparamsData.next(
        {
          ...
          'login_data': resData.return_data.login_data,
          'time_msg': resData.return_data.time_msg
        }
      );
    ...
    private storeAuthData(_data) {
      ...
      this.storage.set('setStroageGlobalParamsData',  {
        ...
        'login_data': _data.return_data.login_data,
        'time_msg': _data.return_data.time_msg
      });
